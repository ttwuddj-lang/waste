...
class YouTube:
    ...
    def _track_from_result(self, data: dict, m_id: int = 0, video: bool = False) -> Track | None:
    ...
    async def search(self, query: str, m_id: int, video: bool = False) -> Track | None:
        try:
            ...
            if results and results.get("result"):
                return self._track_from_result(results["result"][0], m_id, video)
        except Exception as e:
            logger.error(f"Search error: {e}")
        return None

    async def related(self, current: Track, video: bool = False) -> Track | None:
        """Find a fresh next track from YouTube search results using the current song as context."""
        try:
            context = current.title or "music"
            if current.channel_name:
                context = f"{context} {current.channel_name}"
            results = await VideosSearch(context, limit=8).next()
            candidates = results.get("result", []) if results else []
            for data in candidates:
                if data.get("id") == current.id:
                    continue
                track = self._track_from_result(data, 0, video)
                if track and track.duration_sec <= config.DURATION_LIMIT:
                    return track
        except Exception as e:
            logger.error(f"Autoplay related search error: {e}")
        return None
